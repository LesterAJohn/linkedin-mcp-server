...
tools, and a valid JSON invocation example. `get_inbox`, `get_conversation`,
and `search_conversations` are marked as mutating reads because resolving thread
references can visit matching inbox rows and LinkedIn may mark those rows as
read. Outbound actions remain destructive and require an explicit confirmation
workflow.

Messaging conversation references returned by `get_inbox` and `search_conversations` now include row-level metadata captured from the inbox sidebar: `last_activity`, `preview`, `active`, `has_unread_marker`, `row_text`, and `read_state_confidence`. `get_inbox` also returns `inbox_scroll_diagnostics` so workflow summaries can show how many rows were loaded across non-click scroll passes before thread-reference resolution.
...
- Copy the exact thread ID from `/messaging/thread/<thread_id>/`.
- Call `reply_message` with that `thread_id` and `confirm_send=true`.

`send_message` remains useful for profile-driven outreach. For strict in-thread replies, prefer `reply_message` over `send_message`.
...
- `--chrome-path PATH` - Path to Chrome/Chromium executable (for custom browser installations). The Docker image provides `/opt/patchright/chrome` as a stable symlink to the bundled Patchright Chromium binary.
...
In direct HTTP mode, a long-running server exits when it detects that Chromium did not shut down cleanly and the browser profile is still held. Browser shutdown is treated as unconfirmed if Chromium SingletonLock or SingletonSocket state still indicates a live owner after Patchright and Playwright report a successful close. This lets container restart policies replace the process instead of serving repeated `browser is busy` or profile-in-use errors from an unrecoverable profile state.
